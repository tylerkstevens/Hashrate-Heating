...
def calculate_satoshi_revenue(mining_power, start_date, selected_months):
    """Calculates total satoshi revenue from mining based on historical hashvalue and selected months, and plots results."""
    ...
    while current_date <= end_date:
        ...
        cumulative_satoshi_revenue.append(math.ceil(total_satoshis))  # Round up to avoid fractional sats

        current_date += timedelta(days=1)  # Move to the next day

    print(f"Total estimated satoshi revenue from {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}: {math.ceil(total_satoshis):,} sats")
    ...
    plt.title(f"Cumulative Satoshi Revenue ({start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')})\nTotal: {math.ceil(total_satoshis):,} sats")
    plt.legend()
    plt.grid(True)
    plt.show()
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python3 calculate_monthly_satoshi_rev_plot.py <mining_power> <start_date YYYY-MM-DD> <Months Comma-Separated>")
        sys.exit(1)
